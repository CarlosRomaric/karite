@extends('layouts.app')
@section('title') - Création de scéllés @endsection
@section('content')
<div class="container py-5 px-20 mx-auto md:px-20  md:container md:mx-auto content">
        ...
        <div class="pt-10">
            <label for="" class="text-4xl my-5 font-bold">Création de scellés</label>
            ...
            @endif
            <form method="post" action="{{ route('sealeds.store') }}" class="w-full p-10">
                @csrf
                <div class="flex gray-400 mb-6">
                    <div class="w-full md:w-1/3 px-3">
                        ...
                        <div class="relative">
                            <select required class="block appearance-none w-full bg-gray-200 border {{ $errors->has('type') ? 'border-red-500' : 'border-gray-200' }}
                                text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white" id="type" name="type">
                                <option value="">Type de scellés</option>
                                <option value="QR-CODE">Code QR</option>
                                <option value="BAR-CODE">Code-barres</option>
                            </select>
                        ...
                        @if($errors->has('type'))
                            <p class="text-red-500 text-sm">{{ $errors->first('type') }}</p>
                        @endif
                    </div>
                    <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                        ...
                        <div class="relative">
                            <select required class="block appearance-none w-full bg-gray-200 border {{ $errors->has('region_id') ? 'border-red-500' : 'border-gray-200' }}
                    ...
                    <div class="w-full md:w-1/3 px-3">
                        ...
                        <div class="relative">
                            <select required class="block appearance-none w-full bg-gray-200 border {{ $errors->has('number') ? 'border-red-500' : 'border-gray-200' }}
                                text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white
                                " id="number" name="number">
                                <option value="">Sélectionner la nombre de scellés</option>
                        ...
                        @if($errors->has('number'))
                            <p class="text-red-500 text-sm">{{ $errors->first('number') }}</p>
                        @endif
                    </div>
                </div>
                <div class="px-8 mt-8 flex justify-end items-center">
                    <button class="focus:outline-none flex items-center btn-green" type="submit">Générer</button>
                </div>
            </form>
        </div>
</div>
