                        ...
                        <table class="min-w-full text-left text-sm font-light my-10">
                            ...
                            <?php $i=0;?>
                                ...
                                <tr class="border-b border-t-2 border-amber-800 {{ $i % 2 !== 0 ? '' : 'bg-amber-100' }} dark:border-amber-900">
                                    ...
                                    @if(auth()->check() && (auth()->user()->isPlateformAdmin() || auth()->user()->isAgribusinessAdmin ))
                                    <td class="whitespace-nowrap px-6 py-4">{{ number_format($offer->selling_price, 0, ',', ' ') }}</td>
                                    <td class="whitespace-nowrap px-6 py-4">{{ number_format(($offer->selling_price * $offer->weight), 0, ',', ' ') }}</td>
                                    ...
                                    @if(auth()->check() && (auth()->user()->isPlateformAdmin() || auth()->user()->isAgribusinessAdmin ))
                                        ...
                                        @endcanany
                                        <button class="btn-amber-100" wire:click='show("{{$offer->id}}")' >
                                    ...
                                    @else
                                        <button class="btn-success"  wire:click='demande("{{ $offer->id }}")'>
                                            <!-- <svg width="15px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <style type="text/css">
                                                    .st0{fill:#0000;}
                                                </style>
                                        ...
                                        <a href="{{ route('offre.detail', ['id'=>$offer->id]) }}" class="bg-amber-300 text-gray-100 font-semibold py-3 px-4 rounded hover:bg-amber-200">
                                            Détail
                                        </a>
                                    @endif 
